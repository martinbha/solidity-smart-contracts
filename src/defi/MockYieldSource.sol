...
contract MockYieldSource is Ownable {
    ...
    /// @notice The only address allowed to pull yield. Bound once by the owner.
    ...
    ///         vault needs this contract's address at construction. Owner-only
    ///         so the set-once slot cannot be front-run by a stranger between
